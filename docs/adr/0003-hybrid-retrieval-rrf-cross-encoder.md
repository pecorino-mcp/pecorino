...
2. **Dense Vector Search**: Generate local 384-dim embeddings (`Xenova/all-MiniLM-L12-v2`) via ONNX Runtime and compute cosine distance natively in SQLite3 using an HNSW (Hierarchical Navigable Small World) extension (`code_vss_idx`).
...
5. **Heuristic Naming Analysis (LLM)**: An LLM (`ollama/llama3` via LiteLLM) is used as a fallback.
