...
-  **Fast SQLite3 Indexing**: Leverages SQLite3 + `c-fts-uring` for robust metadata storage, hybrid vector search (HNSW), and BM25F full-text search, alongside Gorgonzola for property graph traversal.
...
- **`hybrid`** (default) — Blended vector similarity + SQLite3 FTS with RRF and LTR scoring.
...
3. **Dual Search Engine (Relational, Vector & c-fts-uring) (`src/mcp_server/index_db.py`)**: 
   - **Bi-Encoder Embeddings (`embedding.py`)**: Dense vector representations stored in SQLite3 using HNSW vector extension for fast cosine similarity.
   - **c-fts-uring FTS (`index_db.py`)**: Native SQLite3 C extension using Linux `io_uring` for fast asynchronous BM25F full-text search.
4. **AI Reranking & Learning-to-Rank (`src/mcp_server/cross_encoder.py` & `ltr_ranker.py`)**: 
   - **Cross-Encoder Reranking**: Pairwise ONNX cross-encoder reranking the top candidate results.
   - **LTR Model**: Blends 20+ signals (PageRank, Betweenness Centrality, OOD metrics, Git churn/authorship entropy).
   - **Naming Analysis**: Uses a fallback local LLM for heuristic resolution.
...
- `src/mcp_server/` — MCP server endpoints and core logic.
  ...
  - `src/mcp_server/index_db.py` — SQLite3 and c-fts-uring Full-Text Search (FTS) codebase index.
